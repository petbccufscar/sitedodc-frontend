import React from "react";

const DocentesSubstitutos = (
  <React.Fragment>
    <p>Pessoas que contribuíram para o crescimento e história do DC. Esta lista está em constante elaboração, podendo haver alguns nomes faltantes. Correções e sugestões são sempre bem-vindas.</p>
    <ul>
      <li>Adinovan Henriques de Macedo Pimenta</li>
      <li>Alessandro Marques</li>
      <li>Ana Paula Appel</li>
      <li>Ana Paula do Carmo Marcheti</li>
      <li>André de Souza Tarallo</li>
      <li>Angelo César Colombini</li>
      <li>Antônio Carlos R. da Silva</li>
      <li>Antônio Diogo Hidee Ideguchi</li>
      <li>Antônio Fernando Traina</li>
      <li>Antônio José Manzato</li>
      <li>Aparecida de Cássia Guerra</li>
      <li>Carla Gonçalves da Silva</li>
      <li>Celso Aparecido de França</li>
      <li>Diogo Santos Ortiz Correa</li>
      <li>Edna Yoshiko Senzako</li>
      <li>Ednilson Geraldo Rossi</li>
      <li>Eduardo da Silva Ribeiro</li>
      <li>Eduardo Leal</li>
      <li>Ernane José Xavier Costa</li>
      <li>Fábio Bento Takeda</li>
      <li>Felipe Miney Gonçalves da Costa</li>
      <li>Fernando de Lima Caneppele</li>
      <li>Fernando Genta dos Santos</li>
      <li>Fernando Vernal Salina</li>
      <li>Fernando Vieira Duarte</li>
      <li>Flávio Aldrovandi Montoro</li>
      <li>Giampaolo Luiz Libralon</li>
      <li>Gilmar Cacao Ribeiro</li>
      <li>Guilherme Vaz Torres</li>
      <li>Hendrik Marques Soares</li>
      <li>Isaura Nelsivânia Sombra</li>
      <li>Ivan João Foschini</li>
      <li>Jandira Guenka Palma</li>
      <li>João Luiz Franco</li>
      <li>João Roberto Bertini Junior</li>
      <li>José Alfredo Ferreira Costa</li>
      <li>José Roberto Caon Junior</li>
      <li>Josiel Maimoni de Figueiredo</li>
      <li>Kamila Rios da Hora Rodrigues</li>
      <li>Laís Lemos de Olivera</li>
      <li>Leandro de Carvalho Gonçalves</li>
      <li>Lourenço Alves Pereira Junior</li>
      <li>Luciana Brassolatti</li>
      <li>Luciano Freire</li>
      <li>Luis Gustavo Castanheira</li>
      <li>Luiz Camolesi Junior</li>
      <li>Marcelo da Silva Barreiro</li>
      <li>Marcelo Garcia Manzato</li>
      <li>Márcia Luciana Aguena Castro</li>
      <li>Marco Antônio de Matos Laia</li>
      <li>Marcos Abraão de Souza Fonseca</li>
      <li>Marcos Roberto Bombacini</li>
      <li>Mário Alexandre Gazziro</li>
      <li>Matheus Carvalho Viana</li>
      <li>Maurício Acconcia Dias</li>
      <li>Maurício Falvo</li>
      <li>Mayb Iara Fiats</li>
      <li>Myrian Renata Barros Araújo</li>
      <li>Nassin Chamel Elias</li>
      <li>Olibário José Machado Neto</li>
      <li>Osvaldo Kotaro Takai</li>
      <li>Oswaldo Antônio Beraldo</li>
      <li>Paulo Afonso Parreira Junior</li>
      <li>Paulo Alexandre Bressan</li>
      <li>Pedro Belin Castellucci</li>
      <li>Renê de Souza Pinto</li>
      <li>Ricardo Wagner Campos Martins</li>
      <li>Silvia Mara Abrahão</li>
      <li>Tatiana Silva de Alencar</li>
      <li>Thiago Luís Lopes Siqueira</li>
      <li>Vinícius Fernandes Caridá</li>
      <li>Wesley Willy Oliveira de Souza</li>


    </ul>
  </React.Fragment>
);

export default DocentesSubstitutos;
